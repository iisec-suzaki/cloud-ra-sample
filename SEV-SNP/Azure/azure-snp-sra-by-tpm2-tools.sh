#!/bin/bash
# Azure SEV-SNP Remote Attestation using tpm2_tools (Report Generation) and snpguest (Report Verification)
...
# Prepare the 64-bytes User Data
echo "Prepare 64-byte user data"
openssl rand 64 > user-data.bin

# Write the User Data to vTPM NV Index 0x01400002
echo "Write the User Data to vTPM NV Index 0x01400002"
...
echo "Extract Runtime Claims from the Azure AR"
...
echo "Extract REPORT_DATA from SEV-SNP AR"
dd if=./report.bin \
  skip=80 \
  bs=1 \
  count=64 \
...
## Extract User Data from Runtime Claims
echo "Extract User Data from Runtime Claims"
jq -r '."user-data"' runtime-claims.json | xxd -r -p > user-data-rc.bin

## Verify User Data
echo "Verify User Data"
diff -s user-data.bin user-data-rc.bin
