#!/bin/bash
# Azure TDX Remote Attestation using Trust Authority CLI (Quote Generation) and SGX-DCAP-QvL (Quote Verification)
...
echo "	1. Write H(nonce || raw-user-data) to NV index 0x01400002"
...
sudo mv sgx_default_qcnl_azure.conf /etc/sgx_default_qcnl.conf
...
# Compare HASH(Runtime Claims) with REPORT_DATA in TD Quote by go-tdx-guest
echo "Compare HASH(Runtime Claims) with REPORT_DATA in TD Quote by go-tdx-guest"
go-tdx-guest-check \
	-inform bin \
	-in quote.bin \
...
# Compare SHA512(nonce || rawUserData) with the user-data in the runtime-claims.json
echo "Compare SHA512(nonce || rawUserData) with the user-data in the runtime-claims.json"
